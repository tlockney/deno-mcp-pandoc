...
async function updateMcpTs(newVersion: string): Promise<void> {
  const mcpTsPath = "src/mcp.ts";
  const content = await Deno.readTextFile(mcpTsPath);

  // Find and replace the version in the Server constructor
  const versionRegex = /(\s+version:\s*")([^"]+)(")/;
  const match = content.match(versionRegex);

  if (!match) {
    throw new Error("Could not find version in src/mcp.ts");
  }

  const oldVersion = match[2];
  const updatedContent = content.replace(versionRegex, `$1${newVersion}$3`);

  await Deno.writeTextFile(mcpTsPath, updatedContent);

  console.log(`✓ Updated ${mcpTsPath}: ${oldVersion} → ${newVersion}`);
}
...
async function updateMainTs(newVersion: string): Promise<void> {
  ...
  // Find and replace the version in the health check response
...
async function commitAndTag(version: string): Promise<void> {
  // Add all changes
  await runCommand("git", ["add", "deno.json", "src/mcp.ts", "src/main.ts"]);
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (options.dryRun) {
      ...
      console.log(`Would update src/mcp.ts version to: ${newVersionString}`);
    ...
    await updateMcpTs(newVersionString);
